...
total = 0
for x in range(5):
        r = requests.get(f'http://192.168.0.{x}/get_num_pending_transactions/')
        num_pending = r.json()['num_pending']

        r = requests.get(f'http://192.168.0.{x}/get_num_blocks_created/')
        num_blocks = r.json()['num_blocks']
        total += num_blocks

        r = requests.get(f'http://192.168.0.{x}/get_blockchain_length/')
